...
eroded = cv.erode(
    dilated, (7, 7), iterations=3
)  # get back almost the same as Less Edges
cv.imshow("Eroded", eroded)
...
resized = cv.resize(
    img, (500, 500), interpolation=cv.INTER_CUBIC
)  # better resolution if shape is different from original
